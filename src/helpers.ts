...
export const log = (index: number = 0) => (e: any) => {
    if (index >= 1) console.log(index, JSON.stringify(e, null, 2));
    return e;
  };

export const getUnderlyingTypeAndName = (params: mt.AllTypes): { tag: mt.PRIMITIVE_TYPES, name: string } => {
    switch(params._tag) {
        case "StringType":
        case "BigIntType":
        case "NumberType":
...
export const convertInterfaceType = (it: mt.InterfaceType): mt.InterfaceType => {
...
export const getUnderlyingContext = (params: mt.AllTypes): (mt.PrimitiveType | mt.InterfaceType | mt.PartialType)[] => {
    switch(params._tag) {
        case "StringType":
        case "BigIntType":
        case "NumberType":
